## Section 1: Software and platform section 

The only software needed for this project is a Google Colab Jupyter Notebook. No add-on packages were necessary to install outside of some simple imports in the Jupyter Notebook code. While the code was written on a Windows machine, it should be able to be run on any platform.

## Section 2: A Map of your documentation. 
data:  

    COVID Dataset.csv
    Data Link
    Data Appendix.pdf

output:

    chi_square_cases_by_region.png
    chi_square_deaths_by_region.png
    sarima_cases_comparison_by_region_table.png
    sarima_deaths_comparison_by_region_table.png
    sarima_forecast_plots_by_region_cases.pdf
    sarima_forecast_plots_by_region_deaths.pdf

scripts:

    ALL_CODE_PLUS_OUTPUTS.ipynb
    LICENSE.md
    README.md


## Section 3: Instructions for reproducing your results. 
To reproduce our results, first download the COVID dataset from the Data Link file in the data folder of the repository. Save the dataset locally, and make sure the file name matches the one used in the script. Next, place that data file in the same directory as the Jupyter Notebook located in the scripts folder. Then open the script or notebook and run all code cells in order from top to bottom. When the script finishes running, it will produce the same results shown in our project.
